...
function generateAllocationTable( 
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $class_name = array_key_exists('class_name', $row) ? $row['class_name']: $row['className'];
        $class_code = array_key_exists('class_code', $row) ? $row['class_code']: $row['classCode'];
        $staff = array_key_exists('staff', $row) ? $row['staff']: $row['userID'];
        $allocated = array_key_exists('allocated', $row) ? $row['userID']: $row['userID'];
        ...
        //echo $prefs_object->num_rows . PHP_EOL;

        if ($prefs_object->num_rows > 0) {
            while ($prefs = mysqli_fetch_assoc($prefs_object)) {

                $applicantID = $prefs['userID']; 
                
                // Grab names of all interested applicants for this class code
                $sql2 = "SELECT givenName, familyName FROM systemuser, class WHERE systemuser.userID='$applicantID'";
...
HTML;        
        foreach ($interestedApps as $app) {
            if ($app[2]===$allocated){
                if ($app[2]!='NULL'){
                    $sql = "SELECT userID, givenName, familyName FROM systemUser WHERE systemUser.userID='$allocated' ";
